...
bus = SMBus(0,sda=Pin(0),scl=Pin(1),freq=100000) # FREQUENTIE INSTELLEN op 400kHz IS CRUCIAAL???
time.sleep(1)
devices = bus.scan()
...
def read_gamma():
    ...
    bus.write_byte_data(GDK101_ADDR, 0 ,MEAS1)
    # Wait for the sensor to respond
    time.sleep(0.1)    
    #data = bytes(2)
    # Read the response from the sensor
    data = bus.read_i2c_block_data(self.addr, 0, 2)
    print(data)
